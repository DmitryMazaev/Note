...
def main():
    ...
    while True:
        ...
        command = int(input("1 - создание новой заметки, 2 - поиск заметки, 3 - редактирование заметки, 4 - удаление заметки, 5 - печать заметок: "))
        if command == 1:
            a = note.new_note()
            add_note.add_note(a)
        ...
        if command == 4:
            ...
            command_delete = int(input("1 - по заголовку, 2 - по тексту: "))
            if command_delete == 1:
